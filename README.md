...
####      3.3.3 监听器 
监听器的意思是用来监听测试过程和测试结果。所以主要用来测试结果搜集和展示。 例如我们可以自己通过jsr223
 listner 记录所有采样器执行的请求和结果，便于我们debug使用。 其他经常用的组件， 例如功能测试使用"查看结果树" （性能测试中，建议此监听器关闭或者设置为仅日志错误）。  性能测试中使用 samary report （汇总结果报告） 当然还有各种各样的结果报表和图形展示，利于分析性能测试结果 如TPS ， cpu/内存监控等。
...
3、如果手动指定了cookie，则每次启动测试，cookie Manager不会自动清除。 
...
```
例如线程组设定的用户是50，  集结点设置的用户组数量是25， 那么当集结了25个用户后，就会立即开始执行线程组里的sampler采样器。这就相当于loadrunner的集结点的含义。
```
##### 3.4.3.2 Constant Timer
固定时长的思考时间，思考时间就是设定在某个操作之前进行时间等待。 是否使用以及思考时间的长短依据具体的测试场景。 
